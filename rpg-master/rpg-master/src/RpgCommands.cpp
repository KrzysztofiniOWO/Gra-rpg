...
{
  if (getField(coord).hasTree)
  {
    return 'T';
  }
  else if (getField(coord).hasSpider)
  {
    return 'S';
  }
  else if (getField(coord).hasSkeleton)
  {
    return 'B';
  }
  else if (getField(coord).hasZombie)
  {
    return 'Z';
  }
  else if (getField(coord).hasHero)
  {
    return 'H';
  }
  else if (getField(coord).hasPrincess)
  {
    return 'P';
  }
  else if (getField(coord).hasGold)
  {
    return 'G';
  }
  else
...
void RpgCommands::genTrees() // Generate trees on board
{
  ...
  while (placed < trees)
  {
    ...
    if ((getField(coordinate).hasTree) || (row + 1 >= width) ||
        (col + 1 >= height) || (board[row + 1][col].hasTree) ||
        (getField(coordinate+Coordinates{0, 1}).hasTree))
      continue;
...
void RpgCommands::placeHero()
{
  ...
  while (is_on < 1)
  {
    ...
    if (getField(coordinate).hasTree)
      continue;
...
void RpgCommands::placeMonsters()
{
  ...
  while (are_on < monsters)
  {
    ...
    int col = random % height;
    Coordinates coordinate{row, col};
    if ((getField(coordinate).hasTree) || (getField(coordinate).hasHero))
      continue;
...
void RpgCommands::placePrincess()
{
  ...
  while (is_on < 1)
  {
    ...
    Coordinates coordinate{row, col};
    if ((getField(coordinate).hasTree) ||
        (getField(coordinate).hasHero) ||
        (getField(coordinate).monsterCheck))
      continue;
...
void RpgCommands::placeGoldPiles()
{
  ...
  while (are_on < piles)
  {
    ...
    if ((getField(coordinate).hasTree) ||
        (getField(coordinate).hasHero) ||
        (getField(coordinate).hasPrincess) ||
        (getField(coordinate).monsterCheck))
      continue;
  ...
  else
  {
    ...
    else // Correct move
    {
      ...
      else
      {
        ...
        if (board[coordinates.move_x][coordinates.move_y].hasGold)
        {
          getGold();
          board[coordinates.move_x][coordinates.move_y].hasGold = false;
        }
        else if (board[coordinates.move_x][coordinates.move_y].hasPrincess)
        {
          getPrincess();
          board[coordinates.move_x][coordinates.move_y].hasPrincess = false;
        }
        else if (board[coordinates.move_x][coordinates.move_y].monsterCheck)
        {
...
{
  if (getField(coord).monsterCheck)
  {
